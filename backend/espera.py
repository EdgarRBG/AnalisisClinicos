...
class Espera:
    def registrar(self, nombre='', fecha_nacimiento='', edad=None, telefono='',
                  fecha_cita='', hora_cita='', tipo_estudio='', observaciones=''):
        ...
        try:
            ...
            """, (nombre, fecha_nacimiento, edad, telefono, fecha_cita, hora_cita,
                  tipo_estudio or "Pendiente", observaciones,
    ...
    def actualizar(self, espera_id, nombre, fecha_nacimiento, edad, telefono, observaciones):
        ...
        try:
            cursor.execute("""
                UPDATE espera
                SET nombre=%s, fecha_nacimiento=%s, edad=%s, telefono=%s, observaciones=%s
                WHERE id=%s
    ...
    def marcar_procesado(self, espera_id):
        ...
        try:
            cursor.execute(
                "UPDATE espera SET procesado = 1, estado = 'procesado' WHERE id = %s",
                (espera_id,)
            )
            conn.commit()
            return cursor.rowcount > 0
